...
class MedianFinder:
    ...
    def addNum(self, num):
        self.tree.insertNode(num)
        self.n += 1
    
    def findMedian(self):
        if self.n & 1:
            return self.tree.root.val
        x = self.tree.root.val
        y = self.tree.predecessor(self.tree.root).val
        return (x + y) / 2.0
if __name__ == "__main__":

    sol = [None]
    medianFinder = MedianFinder()
    sol.append(medianFinder.addNum(1))
    sol.append(medianFinder.findMedian())
    sol.append(medianFinder.addNum(2))
    sol.append(medianFinder.findMedian())
    sol.append(medianFinder.addNum(3))
    sol.append(medianFinder.findMedian())
    sol.append(medianFinder.addNum(4))
    sol.append(medianFinder.findMedian())
    sol.append(medianFinder.addNum(5))
    sol.append(medianFinder.findMedian())
    print(sol)